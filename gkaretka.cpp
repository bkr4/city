...
Gkaretka::Gkaretka(Pojazd *pojazd)
{
    obj=pojazd;
...
void Gkaretka::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    QPainterPath path;
    double s=0;
    double c=1;
...
QRectF Gkaretka::boundingRect() const
{
    qreal adjust = 0.5;
    return QRectF(-18 - adjust, -22 - adjust,
                  36 + adjust, 60 + adjust);
}
void Gkaretka::advance(int step)
{
    if (!step)
        return;
    if(obj==nullptr) {
    ...
    if(licznik>8){
        licznik=0;
        if(color==Qt::white) color=Qt::red;
        else color=Qt::white;
    }
    angle=obj->podaj_kat();
